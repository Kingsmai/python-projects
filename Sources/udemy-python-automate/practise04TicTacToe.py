...
def inputPlayerLetter():
    ...
    # Return a list that the player's letter as the first item, and the computer's letter as the second
    letter = ''
...
def playAgain():
    # This function returns True if the player wants to play again, otherwise it will return false
...
def getComputerMove(board, computerLetter):
    ...
    for i in 'top-L top-M top-R mid-L mid-M mid-R low-L low-M low-R'.split():
        dupe = copy.copy(board)
        if isSpaceFree(dupe, i):
            makeMove(dupe, playerLeter, i)
            if isWinner(dupe, playerLeter):
                return i
